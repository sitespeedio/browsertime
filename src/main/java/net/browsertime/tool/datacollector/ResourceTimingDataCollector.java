...
/**
 * Marks defined in the w3c resource timing recommendation.
 * http://www.w3.org/TR/resource-timing/
 * <p/>
 * NOTE: The user timing spec uses a different resolution for time stamps
 * (milliseconds with a decimal part)
 */
public class ResourceTimingDataCollector extends TimingDataCollector {
	private static final String LIST_RESOURCES = "return window.performance.getEntriesByType('resource');";
	...
	public void collectTimingData(JavascriptExecutor js, TimingRun results) {
		...
		if (resources != null && !resources.isEmpty()) {
			// When testing this on IE 11, I can't get hold of the resources
			// as a Map, only the WebRemoteElement
			if (resources.get(0) instanceof Map) {
				for (Map resource : resources) {
					DoubleAdapter da = new DoubleAdapter(resource);
					String name = (String) resource.get("name");
					String initiatorType = (String) resource
							.get("initiatorType");
					...
					double secureConnectionStart = da
							.asDouble("secureConnectionStart");
					...
					results.addResourceMeasurement(new TimingResourceMeasurement(
							name, startTime, initiatorType, duration,
	...
	/**
	 * Helper class to simplify reading time stamps from browsers. Since 0 is
	 * interpreted by Selenium as an integer, it's represented as a Long. This
	 * class avoids ClassCastExceptions when reading all data as doubles.
	 */
