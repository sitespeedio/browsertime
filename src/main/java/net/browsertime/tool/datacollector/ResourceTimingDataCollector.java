...
public class ResourceTimingDataCollector extends TimingDataCollector {
    ...
    public void collectTimingData(JavascriptExecutor js, TimingRun results) {
        ...
        if (resources != null && !resources.isEmpty()) {
            for (Map resource : resources) {
                ...
                double secureConnectionStart = ma.asDouble(ResourceTimingAttributes.secureConnectionStart.name());
                ...
                results.addResourceMeasurement(new TimingResourceMeasurement(name, startTime, initiatorType,
                        duration, redirectStart, redirectEnd,
    ...
    /**
     * A listing that explicitly selects attributes is needed since IE adds constructor as a
     * property of a PerformanceResourceTiming object.
    ...
    private static String buildResourceListingJavascript() {
        ...
        for (ResourceTimingAttributes ra : ResourceTimingAttributes.values()) {
            builder.append("r.").append(ra.name()).append(" = entries[i].").append(ra.name()).append(";").append('\n');
        }
