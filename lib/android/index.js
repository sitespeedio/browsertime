...
export class Android {
  ...
  async measureUsbPowerUsage(startTime, endTime, port) {
    return await getUsbPowerUsage(startTime, endTime, port);
  }
}

async function getUsbPowerUsageForRange(startTime, endTime, port) {
  let resp = await fetch(
      `http://localhost:${port}/power?start=${startTime}&end=${endTime}`
  );
  let respData = await resp.json();
  return respData;
}

async function getUsbPowerUsage(startTime, endTime, port, commands) {
  let baselineUsageData = await getUsbPowerUsageForRange(startTime-2, startTime-1, port);
  let baselineUsageTotal = baselineUsageData[0]["samples"]["data"].reduce(
    (currSum, currVal) => currSum + parseInt(currVal[1]), 0
  );
  let baselineUsage = baselineUsageTotal / baselineUsageData[0]["samples"]["data"].length;

  let powerUsageData = await getUsbPowerUsageForRange(startTime, endTime, port);
