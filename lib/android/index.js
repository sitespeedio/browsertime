...
class Android {
  ...
  async _processStartTime(pid) {
    ...
    const jiffes_per_s = Math.round(
      awk_start_time_after_system_start_time_in_jiffies / system_uptime_in_s
    );

    const process_start_time_in_ms =
      date_in_ms -
      MILLIS_PER_S * system_uptime_in_s +
      (process_start_time_after_system_start_time_in_jiffies * MILLIS_PER_S) /
        jiffes_per_s;
