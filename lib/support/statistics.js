...
module.exports = {
  ...
  formatStatistics: function(timings, percentiles) {
    percentiles = percentiles || [0, 10, 70, 80, 90, 99, 100];
    let data = {};

    Object.keys(timings).forEach(function(timing) {
      let stats = timings[timing];
      let decimals = 0;
      data[timing] = {
        median: stats.median().toFixed(decimals),
        mean: stats.amean().toFixed(decimals)
      ...
      percentiles.forEach(function(p) {
        let name = percentileName(p);
        data[timing][name] = stats.percentile(p).toFixed(decimals);
      });
    });
