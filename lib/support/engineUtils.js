...
export async function loadPrePostScripts(scripts) {
  return toArray(scripts).map(script => {
    ...
    try {
      const myFunction = import(resolve(script));
...
export async function loadScript(script, options, throwError) {
  if (script) {
    ...
    if (options.cjs === false || script.endsWith('.mjs') || script.endsWith('.cjs'))  {
      ...
      try {
        ...
        if (!(await exists(packageJson))) {
          ...
          log.error('Could not create package.json file, the %S file will be treated as a module', script);
        }
