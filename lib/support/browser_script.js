...
function getScriptsForCategories(categories) {
  return Promise.resolve(categories)
    .then((categories) => fs.readdirAsync(rootFolder)
 ...
 * The category name is taken from the directory name, unless a js file is passed as root, in
 * which case it's put into defaultCategory.
 *
 ...
 * @param {string} defaultCategory a category to apply if a file is passed
 * @returns {Promise.<Object>}
 */
function findAndParseScripts(root, defaultCategory) {
  function scriptsFromDirectory(dirpath) {
    return fs.readdirAsync(dirpath)
      .map((name) => toFullPath(name, dirpath))
      .filter(filters.onlyFiles)
  ...
  function parseDirectory(dirPath) {
    ...
    return scriptsFromDirectory(dirPath)
      .then((scripts) => {
        const categories = {};
  ...
  return fs.statAsync(root)
    .then((stat) => {
      if (stat.isFile()) {
        return fs.readFileAsync(root, 'utf8')
          .then((content) => {
            ...
            let categories = {};
            categories[defaultCategory] = scripts;
...
module.exports = {
  defaultScriptCategories: ['browser', 'pageinfo', 'performance'],
  get allScriptCategories() {
    return fs.readdirAsync(rootFolder)
      .map((fileName) => path.join(rootFolder, fileName)).filter(filters.onlyDirectories)
      .map((dirName) => path.basename(dirName));
  },
  getScriptsForCategories: getScriptsForCategories,
  parseScriptDirectory: findAndParseScripts
};
