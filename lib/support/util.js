...
module.exports = {
  ...
  getResultLogLine(result) {
    let totalSize = 0;
    let requests = '';
    let firstPaint = '', domContent = '', pageLoad = '', rumSpeedIndex = '', speedIndex = '', startRender = '', visualComplete85 = '', backEndTime = '', lastVisualChange = '',
    ...
    if (result.statistics.timings && result.statistics.timings.pageTimings) {
      ...
      lastVisualChange = this.formatMetric('lastVisualChange', vm ? vm.LastVisualChange: vm, m, true);
      visualComplete85 = this.formatMetric('visualComplete85', vm ? vm.VisualComplete85: vm, m, true);
      pageLoad = this.formatMetric('Load', pt.pageLoadTime, m, true);
      rumSpeedIndex = this.formatMetric('rumSpeedIndex', t.rumSpeedIndex, m);
      backEndTime = this.formatMetric('BackeEndTime', pt.backEndTime, m, true);
    }
