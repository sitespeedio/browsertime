...
module.exports = {
  ...
  harFromEvents(events, options) {
    ...
    const ongoingDataRequests = new Set(),
    ...
    for (let event of events) {
      ...
      switch (message.method) {
        ...
        case 'Network.requestServedFromCache':
        {
          ...
          if (!entry) {
            log.warn('Received requestServedFromCache for requestId(' + params.requestId + ') with no matching request.');
        ...
        case 'Network.responseReceived':
        {
          ...
          let entry = entries.find((entry) => entry._requestId === params.requestId);
          if (!entry) {
            log.warn('Received network response for requestId(' + params.requestId + ') with no matching request.');
            continue;
          }
        ...
        case 'Network.dataReceived':
        {
          ...
          let entry = entries.find((entry) => entry._requestId === params.requestId);
          if (!entry) {
            log.warn('Received network data for requestId(' + params.requestId + ') for ' + params.request.url + ' with no matching request.');
            continue;
          }
        ...
        case 'Network.loadingFinished':
        {
          ...
          let entry = entries.find((entry) => entry._requestId === params.requestId);
          if (!entry) {
            log.warn('Network loading finished for requestId(' + params.requestId + ') ' + params.request.url + ' with no matching request.');
            continue;
          }
    ...
    entries = entries.filter((entry) => {
      ...
      if (!entry.response && !entry.request.url.endsWith('.ico')) {
        log.warn('Dropping incomplete requestId(' + entry._requestId + '): ' + entry.request.url);
      }
