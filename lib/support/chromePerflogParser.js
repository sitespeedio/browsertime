  ...
  harFromEvents: function(events) {
    function populateEntryFromResponse(entry, response, timestamp) {
      ...
      if (timing) {
        ...
        entry.time = ((blocked == -1) ? 0 : blocked) + ((dns == -1) ? 0 : dns) + ((connect == -1) ? 0 : connect) + send + wait + receive;
    ...
    //Dirty sort since Javascript treats everything as strings
    entries.sort(function(a, b) {
      if (a._requestId.indexOf('r') > -1) return 0;
      let entryOne = parseFloat(a._requestId.split(".").join("0")) * 1000;
      let entryTwo = parseFloat(b._requestId.split(".").join("0")) * 1000;
