...
module.exports = {
  ...
  harFromEvents(events, options) {
    ...
    for (let event of events) {
      ...
      switch (message.method) {
        ...
        case 'Tracing.dataCollected':
          if (blinkUserTiming[message.params.name] === 0) {
            blinkUserTiming[message.params.name] = message.params.ts;
          }
          break;
    ...
    // lets add firstMeaningFulPaint for now, the other are not ok
    if (blinkUserTiming.firstMeaningfulPaint - blinkUserTiming.navigationStart > 0) {
      pages[0].pageTimings._firstMeaningfulPaint = (blinkUserTiming.firstMeaningfulPaint - blinkUserTiming.navigationStart) / 1000;
    }
