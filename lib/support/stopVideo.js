...
function spawnVisualMetrics(data, results, index) {

  let visualMetricsArgs = [process.env.VISUALMETRICS, '--video', data.dir + '/' +  data.name + '-run-' + index + '.mpg', '--orange', '--dir',  data.dir + '/images'  , '--force']
  ...
  return new Promise(function(resolve, reject) {
    ...
    visualMetrics.on('exit', function(code, signal) {
      results.timings.visualMetrics = metrics;
...
module.exports = {
  run(context) {
    // stop(runner, data, results, index) {
    // stop ffmpeg
    if (context.taskData.ffmpeg) {
      context.taskData.ffmpeg.kill('SIGHUP');
    }
    // and start visual metrics
    return spawnVisualMetrics(context.taskData, context.results, context.index);
  }
};
