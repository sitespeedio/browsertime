...
function validateInput(argv) {
  ...
  if (argv.exprimental.video) {
    console.log(process.env.VISUALMETRICS);
    if (process.env.VISUALMETRICS === undefined) {
      return 'You need to set the environment variable VISAUALMETRICS to point out the full path to the /the/full/path/visualmetrics.py';
    }
...
module.exports.parseCommandLine = function parseCommandLine() {
  let argv = yargs
    ...
    .describe('exprimental.video', 'Record a video and collect SpeedIndex. Needs FFMPEG & VISUALMETRICS')
