...
module.exports.parseCommandLine = function parseCommandLine() {
  let validated = yargs
    ...
    .option('chrome.android.deviceSerial', {
      describe: 'Choose which device to use. If you do not set it, first device will be used.',
      group: 'chrome'
    })
    ...
    .option('requestheader', {
      alias: 'r',
      describe: 'Request header that will be added to the request. Add multiple instances to add multiple request headers. Chrome only at the moment.'
    })
    .option('block', {
      describe: 'Domain to block. Add multiple instances to add multiple domains that will be blocked. Chrome only at the moment.'
    })
    .option('cacheClearRaw', {
      describe: 'Use internal browser functionality to clear browser cache between runs instead of only using Selenium. Chrome only at the moment.',
      type: 'boolean',
      default: false
    })
    .option('basicAuth', {
    ...
    .option('preURLDelay', {
      describe: 'Delay between preURL and the URL you want to test (in milliseconds)',
      type: 'integer',
      default: 1500
    })
