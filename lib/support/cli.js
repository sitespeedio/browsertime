...
function validateInput(argv) {
  ...
  if (argv.chrome && argv.chrome.mobileEmulation) {
   ...
   if ((argv.connectivity.profile !== 'native' || argv.connectivity.raw) && argv.connectivity.engine === 'tc') {
   ...
   if ((argv.connectivity.profile !== 'native' || argv.connectivity.raw) && argv.connectivity.engine === 'tsproxy') {
...
module.exports.parseCommandLine = function parseCommandLine() {
  let argv = yargs
    ...
    .option('connectivity.profile', {
      default: 'native',
      choices: ['3g', '3gfast', '3gslow', '2g', 'cable', 'native'],
      describe: 'The connectivity profile. You need to set the engine for this to work.'
    })
    .option('connectivity.raw', {
      default: undefined,
      describe: 'The connectivity raw, a JSON object with the keys downstreamKbps, upstreamKbps and latency. \"{\\\"downstreamKbps\\\":6000, \\\"upstreamKbps\\\": 6000, \\\"latency\\\": 200}\".'
    })
    ...
    .option('connectivity.engine', {
      default: 'tsproxy',
      choices: ['tc','tsproxy'],
      describe: 'The engine for connectivity'
    })
    .option('preScript', {
    ...
    .option('proxy', {
      type: 'boolean',
      default: false,
      describe: 'Use BrowserMobProxy as a proxy to be able to use basicAuth and trafficShaping (but not H/2)'
    })
    .option('resultDir', {
