...
module.exports.parseCommandLine = function parseCommandLine() {
  let validated = yargs
    ...
    .option('chrome.collectCPUMetrics', {
      type: 'boolean',
      describe:
        'Collect CPU metrics. You need to have Python installed + collect the devtools.timeline for this to work.',
      group: 'chrome'
    })
    ...
    .option('video', {
      type: 'boolean',
      describe:
        'Record a video and store the video. Set it to false to remove the video that is created by turning on visualMetrics. Requires FFMpeg to be installed.'
    })
    ...
    .option('pageLoadStrategy', {
      ...
      describe:
        'Setting a page load strategy will cause navigation to be "eager", waiting for the interactive document ready state; "normal" (the default), waiting for the complete ready state; or "none", which will return immediately after starting navigatio.  Note:Chrome do not support "eager". Use this together with pageCompleteCheck to change when you want to end your test.'
    })
    .count('verbose')
    .string('_')
    .help('h')
