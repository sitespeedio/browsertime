...
module.exports.parseCommandLine = function parseCommandLine() {
  let argv = yargs
    ...
    .boolean('proxy')
    .default('delay', 0)
    .default('timeouts.browserStart', 60000)
    .default('timeouts.pageLoad', 120000)
    .default('timeouts.script', 40000)
    .default('timeouts.pageCompleteCheck', 120000)
    .default('proxy', false)
    ...
    .string('chrome.android.package')
    .string('chrome.android.deviceSerial')
    ...
    .describe('experimental.dumpChromePerflog', 'Dump Chromes performance log with Network.* events to disk. [EXPERIMENTAL FEATURE]')
    .describe('experimental.video', 'Record a video and collect SpeedIndex. Needs FFMPEG & Python dependencies for VisualMetrics')
    .describe('browser', 'Specify browser')
    .describe('chrome.android.package', 'Run Chrome on your Android device. Set to com.android.chrome for default Chrome version.')
    .describe('chrome.android.deviceSerial', 'Choose which device to use. If you do not set it, random device will be used.')
    ...
    .describe('proxy', 'Use BrowserMobProxy as a proxy to be able to use basicAuth and trafficShapping (but nut H/2)')
