...
module.exports.parseCommandLine = function parseCommandLine() {
  let validated = yargs
    ...
    .option('connectivity.engine', {
      default: 'external',
      choices: ['tc', 'external', 'throttle'],
      describe:
        'The engine for connectivity. TC (Linux Traffic Control) needs tc work but will only setup upload and latency. Throttle works on Mac and tc based Linux (it is experimental so please use with care). Use external if you set the connectivity outside of Browsertime. The best way do to this is described in https://github.com/sitespeedio/browsertime#connectivity',
      group: 'connectivity'
    })
