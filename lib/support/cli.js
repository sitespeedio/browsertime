...
function validateInput(argv) {
  ...
  if (argv.basicAuth) {
    if (!argv.basicAuth.domain || !argv.basicAuth.username || !argv.basicAuth.username) {
      return 'Basic auth config needs to include both domain, username and password';
    }
  }
  return true;
}
...
module.exports.parseCommandLine = function parseCommandLine() {
  let argv = yargs
    ...
    .describe('basicAuth.domain', 'Domain to apply basic authentication to')
