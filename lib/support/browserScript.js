...
async function scriptsFromDirectory(dirpath) {
  ...
  dir = dir.map(name => toFullPath(name, dirpath));
  dir = dir.filter(await filters.onlyFiles);
  dir = dir.filter(filters.onlyWithExtension('.js'));
  const result = {};
...
async function getScriptsForCategories(categories) {
  let dirs = await readdir(rootFolder);
  dirs = dirs.filter(name => categories.find(n => n === name));
  dirs = dirs.map(name => toFullPath(name, rootFolder));
  dirs = dirs.filter(await filters.onlyDirectories);
  const result = {};
  ...
  get allScriptCategories() {
    return readdir(rootFolder).then(files => {
      ...
      return Promise.all(p).then(isDirs => {
        const onlyDirs = [];
        for (const i = 0; i < isDirs.length; i++) {
          if (isDirs[i]) {
