      ...
      return driver.get('data:text/html;charset=utf-8,')
        ...
        .then((videoDir) => {
          ...
          if (clientId) {
            ...
            return client.shell(clientId, 'screenrecord --bit-rate 8000000 /sdcard/browsertime.mp4')
          } else {
            return client.listDevices()
              .then((devices) => {
                // just take the first phone online
                taskData.deviceid = devices[0].id;
                context.log.info('Start screenrecord on device %s', devices[0].id);
                return client.shell(devices[0].id,
