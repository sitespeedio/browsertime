...
module.exports = {
    run(context) {
        ...
        const minimumDiffLevel = taskData.android ? 50 : 64;
        const videoFile = taskData.videoPaths['' + context.index];
        const tmpFile = path.join(taskData.videoDir, 'tmp.mp4');
        const originalFile = path.join(taskData.videoDir, context.index + '-original.mp4')
        return ffprobe.run(videoFile).then((ffProbeJson) => {
            const firstFrame = findFirstFrame(ffProbeJson, minimumDiffLevel);
        ...
        then(() =>  {
          if (context.options.keepOriginalVideo) {
            return fs.renameAsync(videoFile, originalFile).then(() => fs.renameAsync(tmpFile, videoFile));
          } else {
