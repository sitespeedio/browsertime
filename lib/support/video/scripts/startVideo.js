      ...
      return driver.get('data:text/html;charset=utf-8,')
        ...
        .then((videoDir) => {
          ...
          if (process.platform === 'darwin' && !context.options.xvfb) {
            ...
            const originFirefox = '0,70';
            const originChrome = '0,66';
            // The offset for Firefox is kind of magic numbers. Using XVFB and FF you always get a black border
            // to the left and under the video even though we maximize the browser
            const offsetFirefox = {x: 134, y: 160};
            const offsetChrome = {x: 0, y: 66};
            recording = ffmpegRecorder.startRecordingX11({
              display: 99,
              size: context.options.viewPort,
              filePath: mpegPath,
              origin: context.options.browser === 'firefox' ? originFirefox : originChrome,
              offset: context.options.browser === 'firefox' ? offsetFirefox : offsetChrome
            })
          }
