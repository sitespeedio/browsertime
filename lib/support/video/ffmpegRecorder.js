...
function buildX11FfmpegArgs({display, screen = 0, framerate = 60, origin = '0,0', size, filePath}) {
  return ['-hide_banner',
    '-an',
    '-y',
    '-framerate', framerate,
...
function buildAVFoundationFfmpegArgs({display, framerate = 60, filePath}) {
  return ['-hide_banner',
    '-an',
    '-y',
    '-framerate', framerate,
...
module.exports = {
   ...
   */
  startRecordingX11({display, origin, size, filePath, offset}) {
    const widthAndHeight = size.split('x');
    const withoutTopBar = (parseInt(widthAndHeight[0]) - offset.x) + 'x' + (parseInt(widthAndHeight[1]) - offset.y);
