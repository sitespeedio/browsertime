  ...
  addExtrasToHAR(
    ...
    if (options.resultURL) {
      ...
      if (options.screenshot) {
        _meta.screenshot = `${base}${pathToFolder(url)}screenshots/${index +
          1}.${options.screenshotParams.type}`;
      }
      if (options.video) {
        _meta.video = `${base}${pathToFolder(url)}video/${index + 1}.mp4`;
      }
      if (options.chrome && options.chrome.timeline) {
        _meta.timeline = `${base}${pathToFolder(url)}/trace-${index}.json`;
      }
    }
  }
};
