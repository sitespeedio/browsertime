...
function setupMobProxyDefaults(options) {
    ...
    if (options.connection) {
        ...
        } else if (options.connection === 'mobile3gslow') {
            options.connectionRaw = {
                downstreamKbps: 780,
                upstreamKbps: 330,
                latency: 200
            };
        }  if (options.connection === 'mobile2g') {
           options.connectionRaw = {
               downstreamKbps: 35,
               upstreamKbps: 32,
               latency: 1300
...
var createProxy = function(options) {
  ...
  var argType = typeof useProxy;

  if ((argType === 'string' && useProxy !== 'true') ||
      (argType === 'boolean' && useProxy === false)) {
    return nullProxy;
  }
