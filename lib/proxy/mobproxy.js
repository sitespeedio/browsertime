...
Proxy.prototype.saveHar = function(filename, data, cb) {
  this.proxy.getHAR(this.port, function(err, har) {
    ...
    for (var i = 0; i < theHar.log.pages.length; i++) {
      ...
      theHar.log.pages[i].title = data.default.data.static.documentTitle;
      if (data.default.data.dynamic[i]) {
        theHar.log.pages[i].pageTimings.onContentLoad = data.default.data.dynamic[i].timings.domContentLoadedTime;
        theHar.log.pages[i].pageTimings.onLoad = data.default.data.dynamic[i].timings.pageLoadTime;
      }
    }
