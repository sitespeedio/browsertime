...
Proxy.prototype.saveHar = function(filename, data, cb) {
  this.proxy.getHAR(this.port, function(err, har) {
    ...
    for (var i = 0; i < theHar.log.pages.length; i++) {
      ...
      theHar.log.pages[i].title = data.default.documentTitle;
      if (data.default.data[i]) {
        theHar.log.pages[i].pageTimings.onContentLoad = data.default.data[i].timings.domContentLoadedTime;
        theHar.log.pages[i].pageTimings.onLoad = data.default.data[i].timings.pageLoadTime;
      }
