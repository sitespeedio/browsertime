  ...
  async perIteration(allData, index) {
    for (let data of allData) {
      ...
      if (
        ...
        log.info(
          `${url} BackEndTime: ${pt.backEndTime} DomInteractiveTime: ${pt.domInteractiveTime} DomContentLoadedTime: ${pt.domContentLoadedTime} FirstPaint: ${data.browserScripts.timings.firstPaint} PageLoadTime: ${pt.pageLoadTime}`
        );
      }
      ...
      if (data.visualMetrics) {
        if (this.options.iterations > 1) {
          log.info(
            `VisualMetrics FirstVisualChange: ${data.visualMetrics.FirstVisualChange} SpeedIndex: ${data.visualMetrics.SpeedIndex} PerceptualSpeedIndex: ${data.visualMetrics.PerceptualSpeedIndex} LastVisualChange: ${data.visualMetrics.LastVisualChange}`
          );
        }
