...
class Collector {
  ...
  async perIteration(allData, index) {
    for (let data of allData) {
      // TODO may not be fail safe :)
      const url = data.browserScripts.pageinfo
        ? data.browserScripts.pageinfo.url
      ...
      await this.storageManager.createSubDataDir(
        path.join(pathToFolders(url, this.options.useHash))
      );
      ...
      forEach(data.extraJson, (file, filename) =>
        extraWork.push(
          this.storageManager.writeJson(
            path.join(pathToFolder(data.url), filename),
            file,
            true
          )
        )
      ...
      if (this.options.screenshot) {
        results.files.screenshot.push(
          `${pathToFolder(url, this.options.useHash)}screenshots/${index}.${
      ...
      if (this.options.video) {
        results.files.video.push(
          `${pathToFolder(url, this.options.useHash)}video/${index}.mp4`
        );
      }
      if (this.options.chrome && this.options.chrome.timeline) {
        results.files.timeline.push(
          `${pathToFolder(url, this.options.useHash)}trace-${index}.json.gz`
        );
      }
      if (this.options.chrome && this.options.chrome.collectConsoleLog) {
        results.files.consoleLog.push(
          `${pathToFolder(url, this.options.useHash)}console-${index}.json.gz`
        );
      }
