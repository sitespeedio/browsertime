...
class Collector {
  ...
  async perIteration(allData, index) {
    for (let data of allData) {
      // if we got an alias from Browsertime, use that instead of the URL see
      // https://github.com/sitespeedio/sitespeed.io/issues/2290#issuecomment-461601990
      const name = data.alias ? data.alias : data.url;
      const results = this.allResults[name]
        ? this.allResults[name]
        : getNewResult(data.url, this.options);
      const statistics = this.allStats[name]
        ? this.allStats[name]
        : new Statistics();
      ...
      if (
        ...
        log.info(
          `${name} BackEndTime: ${pt.backEndTime} DomInteractiveTime: ${
      ...
      await this.storageManager.createSubDataDir(
        path.join(pathToFolders(name, this.options))
      );
      ...
      forEach(data.extraJson, (file, filename) =>
        extraWork.push(
          this.storageManager.writeJson(
            path.join(pathToFolder(name, this.options), filename),
            file,
            true
          )
        )
      ...
      if (this.options.screenshot) {
        results.files.screenshot.push(
          `${pathToFolder(name, this.options)}screenshots/${index}.${
