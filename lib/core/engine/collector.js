...
class Collector {
  ...
  /**
   * Collect all individual runs, add it to the stistics, and store
  ...
  async perIteration(allData, index) {
    for (let data of allData) {
      ...
      const url = data.browserScripts.pageinfo.url;
      const results = this.allResults[url]
        ? this.allResults[url]
        : getNewResult(url, this.option);
      ...
      if (this.options.screenshot && data.screenshot) {
        extraWork.push(
          this.screenshotManager.save(`${index}`, data.screenshot)
        );
      }
