...
export default class Iteration {
  constructor(
    ...
    try {
      this.preScripts = loadPrePostScripts(options.preScript);
      this.postScripts = loadPrePostScripts(options.postScript);
      this.postURLScripts = loadPrePostScripts(options.postURLScript);
      this.pageCompleteCheck = options.pageCompleteCheck
        ? loadScript(options.pageCompleteCheck)
        : undefined;
    } catch (error) {
