...
class Iteration {
  constructor(
    ...
    this.myVideo = new Video(options, storageManager);
  }
  ...
  async run(index) {
    ...
    return browser
      .start()
      .then(() => setResourceTiming(taskOptions))
      ...
      .then(() => {
        ...
        if (recordVideo && !combine) {
          return setOrangeBackground(taskOptions)
            .then(() => video.record())
            .then(() => setWhiteBackground(taskOptions));
        } else return Promise.resolve();
      })
