  ...
  async run(navigationScript, index) {
    ...
    try {
      ...
      const context = {
        ...
        driver: browser.getDriver(), // https://seleniumhq.github.io/selenium/docs/api/javascript/module/selenium-webdriver/lib/webdriver_exports_WebDriver.html - webdriver browser session
        h: helpers
      };
      ...
      if (recordVideo) {
        if (noPages > 1) {
          ...
          for (let i = 1; i <= noPages; i++) {
            const data = await video.combine(
              i,
              result[i - 1].browserScripts.pageinfo.visualElements,
              result[i - 1].browserScripts.timings.pageTimings
            );
            result[i - 1].visualMetrics = data.visualMetrics;
          }
          await video.cleanup();
        } else {
          const videoMetrics = await video.postProcessing(
            result[0].browserScripts.timings.pageTimings,
            result[0].browserScripts.pageinfo.visualElements
          );
          // When we support multiple pages, this needs to be changed
          result[0].visualMetrics = videoMetrics.visualMetrics;
        }
      }
      return result;
    } catch (e) {
