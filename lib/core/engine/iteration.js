...
import { GeckoProfilerCommand } from './command/geckoProfilerCommand.js';
  ...
  async run(navigationScript, index) {
    ...
    try {
      ...
      if (options.browser === 'firefox') {
        if (
          options.firefox &&
          options.firefox.geckoProfilerRecordingType === 'custom'
        ) {
          ...
          profiler = new GeckoProfilerCommand(browserProfiler, browser, index);
        }
      } else {
        profiler = undefined;
