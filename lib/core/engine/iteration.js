...
const setResourceTiming = require('../../support/setResourceTiming');
...
class Iteration {
  ...
  run(index) {
    ...
    return video
      .setupDirs(index)
      .then(() => browser.start())
      .then(() => setResourceTiming(browser.getDriver()))
      .then(() => extensionServer.setupExtension(url, browser.getDriver()))
      .then(() => {
      ...
      .then(() => {
        // Access the preURL
        if (options.preURL) {
          return preURL(browser.getDriver(), options);
        } else return Promise.resolve();
      })
      ...
      .then(() => {
        ...
        if (recordVideo && !combine) {
          return setOrangeBackground(browser.getDriver())
            .then(() => video.record())
            .then(() => setWhiteBackground(browser.getDriver()));
        } else return Promise.resolve();
      })
      ...
      .then(() => {
        if (testOnAndroid && options.chrome.collectNetLog) {
          ...
          return android.initConnection().then(() => android.pullNetLog(index));
        } else return Promise.resolve();
      })
      ...
      .then(() => {
        ...
        if (recordVideo) {
          return video.postProcessing(result);
        } else return Promise.resolve();
      })
      .then(() => result)
      .finally(() => {
