...
class Iteration {
  ...
  run(runIndex) {
    ...
    return browser
      .start()
      .then(() => extensionServer.setupExtension(url))
      .then(() => {
      ...
      .then(() => this.runDelegate.onStartIteration(browser, index))
      .then(() => {
      ...
      .then(() => {
        // We should use sharp gere so we can save the image
      ...
      .then(() => this.runDelegate.onStopIteration(browser, index, result))
      .then(() =>
      ...
      .then(() => {
        // maybe we should clode the browser before? hmm
        if (recordVideo) {
          taskOptions.results = result;
