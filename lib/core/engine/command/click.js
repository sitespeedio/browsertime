...
class Click {
  ...
  async byClassName(className) {
    try {
      const script = `document.getElementsByTagName('${className}')[0].click();`;
  ...
  async byClassNameAndWait(className) {
    try {
      ...
      return this.browser.wait(this.pageCompleteCheck, EXTRA_WAIT_TIME);
    } catch (e) {
      log.error('Could not find element by class name %s', className);
      log.verbose(e);
      throw e;
  ...
  async byXpath(xpath) {
    try {
      const script = `document.evaluate("${xpath}", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue.click();`;
      return this.browser.runScript(script, 'CUSTOM');
    } catch (e) {
  ...
  async bySelector(selector) {
    try {
      ...
      log.error('Could click using selector %s', selector);
      log.verbose(e);
      throw e;
    }
  }
