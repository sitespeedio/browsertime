...
export default class Engine {
  ...
  async runByScript(
    ...
    try {
      preScripts = await loadPrePostScripts(options.preScript);
      postScripts = await loadPrePostScripts(options.postScript);
      postURLScripts = await loadPrePostScripts(options.postURLScript);
      pageCompleteCheck = options.pageCompleteCheck
        ? await loadScript(options.pageCompleteCheck)
        : undefined;
    } catch (error) {
