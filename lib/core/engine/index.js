...
class Engine {
  ...
  async run(url, scriptsByCategory, asyncScriptsByCategory) {
    ...
    const collector = new Collector(url, statistics, storageManager, options);
    ...
    try {
      ...
      totalResult.statistics = statistics.summarizeDeep(options);

      // Add extra fields to the HAR
      // to make the HAR files better when we use them in
      // compare.sitespeed.io
      if (totalResult.har) {
        const numPages = totalResult.har.log.pages.length;
