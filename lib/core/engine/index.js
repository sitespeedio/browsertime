...
function addExtraFieldsToHar(totalResults, har, options) {
  if (har) {
    for (let index = 0; index < totalResults.length; index++) {
      for (let run = 0; run < options.iterations; run++) {
        const hepp = index + run;
        const page = har.log.pages[hepp];
        const visualMetric = totalResults[index].visualMetrics[hepp];
        const browserScript = totalResults[index].browserScripts[hepp] || {};
        const cpu = totalResults[index].cpu[hepp] || {};
