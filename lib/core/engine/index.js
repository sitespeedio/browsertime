  ...
  async start() {
    ...
    if (this.options.android) {
      ...
      if (phoneState !== 'device') {
        const message = `The phone state is ${phoneState}`;
        log.error(message);
        throw Error(message);
      }

      await android.clickHomeButton();
      ...
      if (this.options.androidVerifyNetwork) {
        ...
        if (!connection) {
          const message = `No internet connection. Could not ping ${pingAddress}`;
          log.error(message);
          throw Error(message);
        }
      }
        ...
        if (temp > minTempLimit) {
          do {
            ...
            if (batteryTry === maxTries) {
              log.error(
                'Battery temperature (%s) never got under %s after %s tries, rebooting the device.',
                temp,
                minTempLimit,
                maxTries
              );
