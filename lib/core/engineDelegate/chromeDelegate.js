...
class ChromeDelegate {
  constructor({skipHar = false, chrome = {}}) {
    ...
    this.logPerfEntries = !!chrome.collectChromePerflog;
    this.collectTraceCategoriesLog = !!chrome.collectTraceCategoriesLog;
  }
  ...
  onStopIteration(runner, index, results) {
    ...
    return runner.getLogs(webdriver.logging.Type.PERFORMANCE)
      ...
      .tap((messages) => {
        if (this.collectTraceCategoriesLog) {
          const cleaned = traceCategoriesParser.parse(messages);
