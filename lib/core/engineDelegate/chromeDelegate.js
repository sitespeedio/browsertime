...
const Promise = require('bluebird'),
  ...
  timeLineParser = require('../../support/timeLineParser');
...
class ChromeDelegate {
  constructor({skipHar = false, experimental = {}}) {
    this.skipHar = skipHar;
    this.logPerfEntries = !!experimental.dumpChromePerflog;
    this.dumpTimeLineLog = !!experimental.dumpTimeLineLog;
  }
  ...
  onStopIteration(runner, index, results) {
    ...
    return runner.getLogs(webdriver.logging.Type.PERFORMANCE)
      ...
      .tap((events) => {
        if (this.dumpTimeLineLog) {
          const cleaned = timeLineParser.parse(events);
          results.extraJson['timeline-' + index + '.json'] = cleaned;
        }
      })
