...
function loadPrePostScripts(scripts) {
  return util.toArray(scripts).map((script) => {
    try {
      ...
      throw new Error('Couldn\'t load task file: ' + script);
    }
  });
}
...
class Engine {
  ...
  start() {
    if ((this.options.connectivity && this.options.connectivity.profile !== 'native') || (this.options.connectivity && this.options.connectivity.raw)) {
