...
class Engine {
  constructor(options) {
    ...
    this.xvfb = this.setupXvfb();
  }

  setupXvfb() {
    const width = get(this.options, 'chrome.mobileEmulation.width');
    const height = get(this.options, 'chrome.mobileEmulation.height');

    let widthAndHeight = (width && height) ? width + 'x' + height : this.options.viewPort || '1500x1200';
    ...
    return Promise.promisifyAll(xvfb);
  }


  startXvfb() {
    if (this.options.xvfb) {
        return this.xvfb.startAsync();
    }
  ...
  start() {
    ...
    return this.startXvfb().then(connectivity.set(this.options)).then(function() {
      return runDelegate.onStart();
    }).catch(function(e) {
      log.error('Error running browsertime', e);
      throw e;
    });
  } else return this.startXvfb().then(runDelegate.onStart());
  }
  ...
  stop() {
    const runDelegate = this.runDelegate;
    if (this.options.connectivity && this.options.connectivity.profile !== 'native' && this.options.connectivity.profile !== undefined) {
      return this.stopXvfb().then(connectivity.remove(this.options)).then(function() {
        return runDelegate.onStop()
      });

    }
      else return this.stopXvfb().then(runDelegate.onStop());
  }
}
