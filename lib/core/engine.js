...
class Engine {
  ...
  startExtensionServer() {
    return extensionServer.startServer()
  ...
  startXvfb() {
    if (this.options.xvfb) {
      return xvfb.startXvfb({size: this.options.viewPort, options: this.options})
        .tap((xvfbSession) => {
  ...
  start() {
    return Promise.all([this.startXvfb(), connectivity.set(this.options), this.startExtensionServer()]);
  }
  ...
  run(url, scriptsByCategory, asyncScriptsByCategory) {
    ...
    if (options.cacheClearRaw || options.requestheader || options.block || options.basicAuth) {
    ...
    function runIteration(index) {
      ...
      return Promise.resolve({
        ...
        .tap((results) => {
          ...
          runner.getLogs(webdriver.logging.Type.BROWSER)
            .tap((browserLog) =>
              results.browserLog = browserLog)
          }
        })
        .tap((results) => {
        ...
        Promise.reduce(iterations, (results, item, runIndex, totalRuns) => {
          let promise = runIteration(runIndex)
            .then((iterationData) => {
              ...
              if (iterationData.browserLog) {
                results.consoleLog.push(iterationData.browserLog);
              }
  ...
  stop() {
    return Promise.all([this.stopXvfb(), connectivity.remove(this.options), this.stopExtensionServer()]);
  }
}
