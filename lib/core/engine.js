...
let Promise = require('bluebird'),
  ...
  startVideo = require('../support/startVideo').start,
  stopVideo = require('../support/stopVideo').stop,
  ...
  run(url, scriptsByCategory, asyncScriptsByCategory) {
    ...
    function runIteration(index) {
      ...
      return runner.start()
        ...
        .tap(() => {
          if (options.exprimental.video) {
            return startVideo(runner, url, options, videoData, index);
          }
        })
        ...
        .tap((results) => {
          if (options.exprimental.video) {
            return stopVideo(runner, videoData, results, index);
          }
        })
