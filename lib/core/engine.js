    ...
    return Promise.resolve({
      ...
      .tap(result => {
        for (let run = 0; run < result.har.log.pages.length; run++) {
          const harPage = result.har.log.pages[run];
          const _meta = (harPage._meta = {});
          _meta.connectivity = options.connectivity.profile;
          const harPageTimings = harPage.pageTimings;

          const _visualMetrics = (result.har.log.pages[
            run
          ]._visualMetrics = {});

          const vs = result.visualMetrics[run];
          const timings = result.browserScripts[run].timings;

          if (vs) {
            harPageTimings._firstVisualChange = vs.FirstVisualChange;
            harPageTimings._lastVisualChange = vs.LastVisualChange;
            harPageTimings._visualComplete85 = vs.VisualComplete85;
            _visualMetrics.FirstVisualChange = vs.FirstVisualChange;
            _visualMetrics.SpeedIndex = vs.SpeedIndex;
            _visualMetrics.VisualComplete85 = vs.VisualComplete85;
            _visualMetrics.LastVisualChange = vs.LastVisualChange;

            _visualMetrics.VisualProgress = jsonifyVisualProgress(
              vs.VisualProgress
            );
          } else if (timings.firstPaint) {
