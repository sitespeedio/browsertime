  ...
  run(url, scriptsByCategory) {
    ...
    function runScript(runner, script) {
      ...
      return Promise.resolve(script)
        .then((script) => ('return ' + script))
        .then((source) => runner.runScript(source));
    }

    function runScriptInCategory(runner, category) {
      return Promise.reduce(Object.keys(category), (results, scriptName) => {
        const script = category[scriptName];
        return runScript(runner, script)
          .then((result) => {
    ...
    function runScripts(runner, scriptsByCategory) {
      return Promise.resolve(scriptsByCategory)
        .then((scripts) => {
          ...
          return Promise.reduce(categoryNames, (results, categoryName) => {
            ...
            return runScriptInCategory(runner, category)
              .then((result) => {
    ...
    function runIteration(index) {
      ...
      return runner.start()
        ...
        .then(() => runScripts(runner, scriptsByCategory))
