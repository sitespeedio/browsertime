...
const defaultPageCompleteCheck = 'return (function() {try { var end = window.performance.timing.loadEventEnd;' +
...
class SeleniumRunner {
  ...
  start() {
    ...
    return startBrowser.call(this)
      .catch((e) => {
        log.info('Browser failed to start, trying one more time. %s', e);
        return startBrowser.call(this)
          .catch((e) => {
      ...
      .tap(() => {
        let timeouts = this.driver.manage().timeouts(),
          ...
          .catch(function() {
            ...
            log.error('Could not set pageLoadTimeout see https://github.com/sitespeedio/browsertime/issues/316');
          })
      ...
      .tap(() => {
        ...
        if (viewPort) {
          ...
          // special handling for FF in XVFB
          // the window size gets larger than screen and we miss the right
          // side without this hack

          if (viewPort === 'maximize' || this.options.browser === 'firefox' && this.options.xvfb) {
  ...
  loadAndWait(url, pageCompleteCheck) {
    ...
    .catch(WebDriverError, (e) => {
        ...
        log.info('Catched a WebDriverError [' + e.message + ']. Try one more time.' + e);
        // wait one second and test again, hopefully everything is ok?!
        return Promise.delay(1000).then(getUrl)
            .then(confirmUrlSuccessfullyLoaded)
            .then(waitForPageCompletion)
            .catch(WebDriverError, (e) => {
