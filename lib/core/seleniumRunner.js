...
class SeleniumRunner {
  ...
  runScriptFromCategory(script, isAsync, name) {
    ...
    return Promise.resolve(script).then(script => {
      if (isAsync) {
        ...
        return this.runAsyncScript(source, name).then(result => {
          if (result.error) {
            throw result.error;
          } else {
  ...
  runScripts(scriptsByCategory, isAsync) {
    ...
    return Promise.resolve(scriptsByCategory).then(scripts => {
      ...
      return Promise.reduce(
        ...
        (results, categoryName) => {
          ...
          return this.runScriptInCategory(category, isAsync).then(result => {
  ...
  runScriptInCategory(category, isAsync) {
    return Promise.reduce(
      Object.keys(category),
      (results, scriptName) => {
        const script = category[scriptName];
        return this.runScriptFromCategory(script, isAsync, scriptName).then(
          result => {
