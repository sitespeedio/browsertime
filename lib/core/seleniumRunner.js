...
class SeleniumRunner {
  ...
  async wait(pageCompleteCheck) {
    ...
    try {
      ...
      log.error('Failed running the PageCompleteCheck:' + e.message);
      throw new UrlLoadError('Failed running the PageCompleteCheck ', {
        cause: e
      });
    }
  }
    ...
    for (let i = 0; i < tries; ++i) {
      try {
        ...
        if (this.options.spa) {
          ...
          // TODO should we throw an error if we fail the last time?
        }
      } catch (e) {
        log.info(
  ...
  async takeScreenshot() {
    try {
      ...
      log.warning('Failed to take screenshot', e);
