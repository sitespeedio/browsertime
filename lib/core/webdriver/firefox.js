...
module.exports.configureBuilder = function(builder, options) {
  ...
  // https://github.com/SeleniumHQ/selenium/issues/3291 needs to be fixed before it works without
  // patching Selenium
  if (options.cacheClearRaw || options.requestheader || options.block) {
    profile.addExtension(path.resolve(moduleRootPath, 'vendor', 'browsertime-0.4.0-an+fx.xpi'));
  }
  ...
  for (const pref of userPrefs) {
      ...
      if (nameAndValue.length === 2) {
          const value = nameAndValue[1] === 'false' ? false : (nameAndValue[1] === 'true' ? true : nameAndValue[1]);
