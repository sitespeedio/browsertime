...
function migrateLegacyOption(options, from, to) {
  if (get(options, from)) {
    log.warn(`The option ${from} has been renamed to ${to}. The old option will be removed in a future version.`);
    set(options, to, true);
  }
}
...
module.exports.configureBuilder = function(builder, options) {
  ...
  if (!chromeConfig.disableWebExtensions) {
    chromeOptions.addExtensions(path.resolve(moduleRootPath, 'vendor', 'browsertime-extension.crx'));
  }
  ...
  if (chromeConfig.collectNetLog) {
    ...
    chromeOptions.addArguments(`--log-net-log=${dir}/chromeNetlog-${options.index}.json`);
  ...
  if (connectivity.engine === 'tsproxy' &&
    ...
    connectivity.profile !== 'native' && !connectivity.tsproxy.bind) {
