...
let Promise = require('bluebird'),
  ...
  geckodriver = require('soprano-saxophone-temp').geckodriver,
...
module.exports.createWebDriver = function(options) {
  ...
  switch (browser) {
    case 'chrome':
    {
      ...
      if ((options.connectivity && options.connectivity.engine && options.connectivity.engine === 'tsproxy') && options.connectivity.profile !== 'native') {
        chromeOptions.addArguments("--proxy-server=socks5://localhost:" + options.connectivity.tsproxy.port);
        chromeOptions.addArguments("--host-resolver-rules=\"MAP * 0.0.0.0 , EXCLUDE localhost\"");
      }
    ...
    case 'firefox': {
      ...
      if ((options.connectivity && options.connectivity.engine && options.connectivity.engine === 'tsproxy') && options.connectivity.profile !== 'native') {
        ...
        profile.setPreference("network.proxy.socks_port", options.connectivity.tsproxy.port);
      ...
      // ugly hack that only works on Linux/Mac for geckodriver
