  ...
  stop() {
    if (this.driver) {
      Promise.resolve(this.driver.manage().logs().get(webdriver.logging.Type.PERFORMANCE))
        ...
        //.tap((events) => Promise.promisifyAll(require('fs').writeFileAsync('events.json', JSON.stringify(entries), 'utf-8')))
        .then((events) => perflogParser.harFromEvents(events))
        .then((har) => {
          console.log(JSON.stringify(har, 2));
          //return Promise.promisifyAll(require('fs').writeFileAsync('chrome.har', JSON.stringify(har.null, 2), 'utf-8'));
        })
        .then(() => {
          return Promise.try(() => {
