...
let Promise = require('bluebird'),
  log = require('intel'),
  path = require('path'),
  JarLauncher = require('../support/jarLauncher'),
...
class BmpRunner {
    ...
    this.jarLauncher = new JarLauncher(
      {
        ...
        'stdoutLogLevel': log.TRACE,
        'stderrLogLevel': log.TRACE,
        'startupCriteria': {
          'success': {
            'stdout': null,
  ...
  start() {
    return getport()
      ...
      .timeout(20000, this.jarPath + ' hasn\'t started after 20 seconds');
  }
  ...
  setLimit(limit) {
    log.debug('Setting connectivity limit %:1j', limit);
