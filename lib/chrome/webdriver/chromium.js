  ...
  async afterEachURL(runner, index, result) {
    ...
    if (
      ...
      if (!this.options.skipHar) {
        // The HAR for the page we are processing is the one pushed last:
        // afterPageCompleteCheck runs before afterEachURL in the same
        // stop flow. Indexing by iteration number breaks as soon as a
        // script measures more than one page per iteration.
        for (let harRequest of this.hars.at(-1).log.entries) {
        ...
        const harPage =
          this.hars.at(-1).log.pages && this.hars.at(-1).log.pages[0];
