...
module.exports.configureBuilder = function(builder, baseDir, options) {
  ...
  if (
    options.connectivity.engine === 'tsproxy' &&
    options.connectivity.tsproxy &&
    options.connectivity.profile !== 'native'
  ) {
    if (!options.connectivity.tsproxy.bind) {
      ...
      chromeOptions.addArguments(
        '--proxy-server=socks5://localhost:' + options.connectivity.tsproxy.port
      );
      chromeOptions.addArguments(
      ...
      chromeOptions.addArguments(
        '--proxy-server=socks://' +
          options.connectivity.tsproxy.bind +
          ':' +
          options.connectivity.tsproxy.port
      );
    }
  }
