...
class ChromeDelegate {
  constructor(storageManager, options) {
    ...
    this.dataCollectionFunction = false;
  ...
  async _startTrace() {
    this.events = [];
    // https://github.com/sitespeedio/sitespeed.io/issues/2513
    if (!this.dataCollectionFunction) {
      this.dataCollectionFunction = this.cdpClient.Tracing.dataCollected(
        ({ value }) => {
