...
class ChromeDelegate {
  ...
  async _startTrace() {
    ...
    this.Tracing = this.client.Tracing;
    this.events = [];
    this.Tracing.dataCollected(({ value }) => {
      this.events.push(...value);
    });

    this.isTracing = true;
    return this.Tracing.start({
      traceConfig: {
  ...
  async onStartIteration(runner) {
    ...
    if (this.chrome.cdp && this.chrome.cdp.performance === true) {
      await runner.sendDevToolsCommand('Performance.enable');
    }
    ...
    */
    ...
    if (this.chrome.collectLongTasks) {
      ...
      await runner.sendDevToolsCommand(
    ...
    if (this.chrome.CPUThrottlingRate && this.chrome.CPUThrottlingRate > 1) {
      ...
      await runner.sendDevToolsCommand('Emulation.setCPUThrottlingRate', {
  ...
  async onCollect(runner, index, result) {
    if (this.collectTracingEvents) {
      ...
      await this.Tracing.end();
      await this.Tracing.tracingComplete();
      this.isTracing = false;
    }
    ...
    const rawCDPMetrics = await runner.sendAndGetDevToolsCommand(
    ...
    if (this.chrome.includeResponseBodies === 'html') {
      const resourceTree = await runner.sendAndGetDevToolsCommand(
        'Page.getResourceTree'
      );
      const url = resourceTree.frameTree.frame.url;
      const html = await runner.sendAndGetDevToolsCommand(
