...
class ChromeDelegate {
  ...
  async afterBrowserStart() {
    ...
    this.cdpClient = ChromeDevtoolsProtocol(this.options);
    return this.cdpClient.setup();
  }

  async onStartIteration(runner) {
    if (this.options.injectJs) {
      await this.cdpClient.injectJavaScript();
    }

    if (this.options.block) {
      await this.cdpClient.blockUrls();
    }
    ...
    if (this.options.basicAuth) {
      await this.cdpClient.setBasicAuth();
    }

    if (this.options.requestheader) {
      await this.cdpClient.setRequestHeaders();
    }
    ...
    if (this.chrome.CPUThrottlingRate && this.chrome.CPUThrottlingRate > 1) {
      await this.cdpClient.setupCPUThrottling();
    }
  ...
  async beforeEachURL(url) {
    if (this.options.cookie && url) {
      await this.cdpClient.setCookies(url);
  ...
  async beforeCollect() {
    if (this.collectTracingEvents) {
      ...
      this.isTracing = false;
      return this.cdpClient.stopTrace();
    }
  }
  ...
  async onCollect(runner, index, result) {
    ...
    if (
      ...
      this.cdpClient.setResponseBodies(har);
    }
