...
class ChromeDelegate {
  ...
  async onStartIteration(runner) {
    if (this.chrome.includeResponseBodies) {
    ...
    if (this.options.block) {
      ...
      await this.cdpClient.Network.enable();
      await this.cdpClient.Network.setBlockedURLs({ urls: block });
    }

    if (this.options.cacheClearRaw) {
      await this.cdpClient.Network.enable();
      await this.cdpClient.Network.clearBrowserCache();
      await this.cdpClient.Network.clearBrowserCookies();
    }
