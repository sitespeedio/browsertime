...
module.exports = {
  async parseCpuTrace(tracelog, url) {
    try {
      log.verbose('Start parse Chrome trace log using Tracium.');
      const tasks = Tracium.computeMainThreadTasks(tracelog, {
        flatten: true
      });
      log.verbose('Finished parsing the trace log using Tracium.');
      ...
      log.verbose('Chrome trace log finished parsed and sorted.');
