...
class Video {
  ...
  async setupDirs(index) {
    ...
    this.index = index;
    return storageManager.createSubDataDir('video').then(myPath => {
      this.videoDir = myPath;
      this.videoPath = path.join(myPath, index + '.mp4');
      return storageManager.createSubDataDir(
