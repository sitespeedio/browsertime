...
const split = require('./postprocessing/visualmetrics/split');
const combine = require('./postprocessing/visualmetrics/combine');
...
class Video {
  ...
  async split() {
    return split(
      this.videoPath,
      this.videoDir,
      get(this.options, 'videoParams.fullSizeVideo', false)
    );
  }

  async combine(pageNumber) {
    return combine(this.videoDir, pageNumber, this.index);
  }
