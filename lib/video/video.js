...
class Video {
  ...
  async setupDirs(index, url) {
    ...
    this.videoDir = await storageManager.createSubDataDir(
      path.join(pathToFolders(url, this.options.useHash), 'video')
    );
    await storageManager.createSubDataDir(
      path.join(
        pathToFolders(url, this.options.useHash),
        'video',
        'images',
        '' + index
      )
   ...
   */
  async record(url, pageNumber, index) {
    this.pageNumber = pageNumber;
    await this.setupDirs(index, url);
