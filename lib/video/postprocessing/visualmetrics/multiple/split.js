 ...
 */
module.exports = async (videoPath, videoDir, index, options) => {
  log.info('Split the video into screenshots');
  const scriptArgs = [
    '--video',
    videoPath,
    '--multiple',
  ...
  if (options.verbose >= 1) {
    ...
    scriptArgs.push(path.join(videoDir, index + '.split.visualmetrics.log'));
