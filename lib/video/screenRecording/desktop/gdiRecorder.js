...
async function createTempFile() {
  ...
  return path.join(dir, 'gdi.mp4'); // Avant x11.mp4
}

module.exports = class gdiRecorder {
  ...
  constructor(options) {
    ...
    this.browserName = options.browser === 'firefox' ? 'Mozilla Firefox' : 'Google Chrome';
  }

  async start() {
    this.filePath = await createTempFile();

    this.recording = ffmpegRecorder.startRecordingGdi({
