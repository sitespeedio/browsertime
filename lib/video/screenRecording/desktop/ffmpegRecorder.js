...
const getScreenOnOSX = require('./getScreen');
const getResolution = require('./getResolution');
...
async function startRecording(ffmpegArgs, nice, filePath) {
  async function waitForRecording(readableStream) {
    log.debug('Waiting for recording');
  ...
  return Promise.race([
    ffmpegProcess,
    waitForRecording(ffmpegProcess.stderr),
    timeout
  ])
    .then(() => {
