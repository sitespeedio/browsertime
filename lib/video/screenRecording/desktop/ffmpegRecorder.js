...
function buildGdiFfmpegArgs({ browserName, framerate = 30, filePath }) {
  return [
    '-f',
    'gdigrab',
    '-framerate',
    ...
    '0',
    '-i',
    'title=SiteSpeed - ' + browserName, //'title=SiteSpeed - Google Chrome' or 'title=SiteSpeed - Mozilla Firefox',
    '-vcodec',
    'libx264',
    '-crf',
    0,
...
module.exports = {
   ...
   * @returns A promise for a recording object. Pass it to stopRecording.
   */
  async startRecordingGdi({ browserName, framerate, nice, filePath }) {
    const ffmpegArgs = buildGdiFfmpegArgs({
      browserName,
      framerate,
      filePath
    });
