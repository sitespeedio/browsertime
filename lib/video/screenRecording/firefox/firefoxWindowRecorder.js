...
function findRecordingStartTime(baseDir) {
  let startTime = undefined;
  fs.readdirSync(baseDir).forEach(file => {
    if (file.startsWith('windowrecording-')) {
      startTime = file.split('-')[1];
    }
  });
  return startTime;
}

function findTimeToFirstFrame(baseDir) {
  ...
  if (files.length < 1 || files === undefined) {
    log.error('Could not find recordings in ' + directoryName);
  } else {
    return files[0].split('-')[2].split('.')[0];
  }
}
...
module.exports = class FirefoxWindowRecorder {
  ...
  async stop(destination) {
    ...
    this.recordingStartTime = findRecordingStartTime(this.baseDir);
    this.timeToFirstFrame = findTimeToFirstFrame(this.baseDir);
