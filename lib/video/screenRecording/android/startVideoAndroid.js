    ...
    return (
      driver
        ...
        .then(videoDir => {
          ...
          const clientId = get(context.options, 'chrome.android.deviceSerial');

          if (clientId) {
            taskData.deviceid = clientId;
            context.log.info('Start screen recording on device %s', clientId);
            return client
              .shell(clientId, 'echo $EXTERNAL_STORAGE')
              .then(adb.util.readAll)
              .then(output => (taskData.sdcard = output.toString().trim()))
              .then(() =>
                client.shell(
                  clientId,
            ...
            return client.listDevices().then(devices => {
              ...
              return client
                .shell(devices[0].id, 'echo $EXTERNAL_STORAGE')
                .then(adb.util.readAll)
                .then(output => (taskData.sdcard = output.toString().trim()))
                .then(() =>
                  client.shell(
