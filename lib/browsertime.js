          ...
          driver.wait(function() {
              ...
              pageLoadTimeout
                ...
                Object.keys(self.scripts).forEach(function(scriptName) {
                  promises.push({name: scriptName, promise: driver.executeScript(self.scripts[scriptName])});
                });
...
Browsertime.prototype._getFormattedResult = function(options) {
  ...
  this.result.forEach(function(run) {
    ...
    self.userDefinedScriptNames.forEach(function(scriptName) {
      self.userDefinedMetrics.push({name: scriptName, value: run[scriptName]});
    });
  ...
  return {
    ...
    userDefinedMetrics: this.userDefinedMetrics,
    data: this.result
  };
};
  ...
  scriptRoots.forEach(function(rootPath) {
    fs.readdirSync(rootPath).forEach(function(file) {
      ...
      if (self.scripts[name]) {
        ...
        self.scripts[name] = '' +fs.readFileSync(path.join(rootPath, file));
      }
