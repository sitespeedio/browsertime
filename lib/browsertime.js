...
function help() {
  ...
  console.log(' --useProxy use the proxy or not');
}
  ...
  if (options.useProxy) {
    ...
    mobProxy.start(function() {
      for (var i = 0; i < options.runs; i++) {
        work.push(self._addWork(mobProxy, options, scripts, result));
      }
      ...
      async.series(work,
        function(err, results) {
          var data = self._fetchData(options, result, results);
          self._saveToFile(data, options, mobProxy, cb);
        });
    });
  } else {
    for (var i = 0; i < options.runs; i++) {
      work.push(self._addWork(undefined, options, scripts, result));
    }
...
BT.prototype._addWork = function(mobProxy, options, scripts, result) {
...
BT.prototype._fetchData = function(options, result, results) {
  var self = this;
...
BT.prototype._saveToFile = function(data, options, mobProxy, cb) {
