...
function Browsertime(b) {
  this.log = winston.loggers.get('browsertime');
  ...
  return function(callback) {
    async.series([
        ...
        function(err, results) {
          ...
          self.log.info('Fetching ' + options.url + ' (' + iteration + ' of ' + totalIterations + ')');
          ...
          driver.wait(function() {
              ...
              function() {
                driver.quit().thenFinally(function() {
                  return callback(new Error('The url ' + options.url + ' timed out'));
                });
              });
        });
  };
