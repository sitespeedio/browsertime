  ...
  if (options.useProxy) {
    ...
    mobProxy.start(function() {
      for (var i = 0; i < options.runs; i++) {
        work.push(self._addWork(mobProxy, options));
      }
      ...
      async.series(work,
        function(err, results) {
          var data = self._fetchData(options, results);
          self._saveToFile(data, options, mobProxy, cb);
        });
    });
  } else {
    for (var i = 0; i < options.runs; i++) {
      work.push(self._addWork(undefined, options));
    }
...
Browsertime.prototype._addWork = function(mobProxy, options) {
...
Browsertime.prototype._fetchData = function(options, results) {
  var self = this;
