...
Browsertime.prototype._killRunningChromeDriver = function() {

	var self = this;
  var exec = require('child_process').exec;

  var pgrep = exec('pgrep chromedriver',
    function(error, stdout, stderr) {
      if (error !== null) {
        // no chromedriver is running
      } else {
        var pkill = exec('pkill -9 chromedriver',
          function(error, stdout, stderr) {
            if (error === null) {
              self.log.info('Killed Chromedriver instance');
            }
						else {
							self.log.info('Couldn\'t kill Chromedriver instance:' + util.inspect(error));
						}
          });
        }
    });
