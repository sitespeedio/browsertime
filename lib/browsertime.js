			...
			function(err) {
				...
				driver.wait(function() {
					...
					pageLoadTimeout
						...
						var promises = [];
						self._populatePromises(driver, self.defaultScripts.dynamic, promises, false, false);
						self._populatePromises(driver, self.customScripts.dynamic, promises, false, true);

						if (iteration === 1) {
							self._populatePromises(driver, self.defaultScripts.static, promises, true, false);
							self._populatePromises(driver, self.customScripts.static, promises, true, true);
						}
...
Browsertime.prototype._getFormattedResult = function(options) {
	...
	this.result.builtin.dynamic.forEach(function(run) {
		Object.keys(self.defaultScripts.dynamic).forEach(function(scriptName) {
			stats.setupTimingsStatistics(defaultDynamic, run);
			stats.setupUserTimingsStatistics(defaultDynamic, run);
			// for all scripts that return numbers automatically
			// include them in the statistics
			if (helper.isNumber(run[scriptName])) {
				stats.setupStatistics(defaultDynamic, run, scriptName);
			}
		});
	...
	return {
		...
		builtin: {
			statistics: stats.formatStatistics(defaultDynamic),
...
Browsertime.prototype._populateScripts = function(options) {
	...
	helper.readScripts(runOnceScriptRoots, self.defaultScripts.static, this.log);
	helper.readScripts(scriptRoots, self.defaultScripts.dynamic, this.log);
};

Browsertime.prototype._populatePromises = function(driver, scripts, promises, isStatic, isCustom) {
