...
var scriptNames = [];
var userDefinedScriptNames = [];
var userDefinedScriptsResult = [];
...
Browsertime.prototype._getFormattedResult = function(options) {
  ...
  this.result.forEach(function(run) {
    scriptNames.forEach(function(script) {
      ...
      if (isNumber(run[script])) {
        stats.setupStatistics(timings, run, script);
      }
    });

    userDefinedScriptNames.forEach(function(script) {
      userDefinedScriptsResult.push({name: script, value: run[script]});
    });
  ...
  return {
    ...
    userDefinedScriptsResult: userDefinedScriptsResult,
    data: this.result
  };
};
...
Browsertime.prototype._populateScripts = function(options) {
  ...
  if (options.scriptPath) {
    ...
    fs.readdirSync(path.resolve(options.scriptPath)).forEach(function(file) {
      userDefinedScriptNames.push(path.basename(file, '.js'));
    });
      ...
      self.scripts.push(
          {
            ...
            script: '' +fs.readFileSync(path.join(rootPath, file))
          }
      );
      scriptNames.push(path.basename(file, '.js'));
    });
  });
};
