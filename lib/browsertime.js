...
Browsertime.prototype.fetch = function(options, cb) {
  ...
  if (options.useProxy === true) {
    ...
    log.log('info', 'Start the proxy [' + options.mobProxyPort + '], will wait ' + options.proxySleepBeforeStart +
      ' ms');
    var self = this;
    mobProxy.start(function() {
      self._do(mobProxy, options, cb);
    });
  }
  // if we don't use the proxy
  else {
    this._do(undefined, options, cb);
  }
};

Browsertime.prototype._do = function(mobProxy, options, cb) {
  var browserRuns = [];
  var self = this;
  for (var i = 0; i < options.runs; i++) {
    browserRuns.push(self._collectFromTheBrowser(mobProxy, options));
  }
...
Browsertime.prototype._collectFromTheBrowser = function(mobProxy, options) {
  ...
  return function(callback) {
    async.series([
      ...
      function(err, results) {
        var driver = require(path.join(__dirname, 'browsers', options.browser)).getDriver(options);
