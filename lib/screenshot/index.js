...
async function savePng(name, data, url, storageManager, config, useHash) {
  ...
  return storageManager.writeData(
    `${name}.png`,
    buffer,
    path.join(pathToFolder(url, useHash), SCREENSHOT_DIR)
  );
}

async function saveJpg(name, data, url, storageManager, config, useHash) {
  const buffer = await sharp(data)
