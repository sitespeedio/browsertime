...
class ExtensionServer {
  ...
  async setupExtension(url, browser, options) {
    ...
    if (
      (options.cacheClearRaw && options.browser === 'firefox') ||
      (options.requestheader && options.browser === 'firefox') ||
      (options.block && options.browser === 'firefox') ||
      (options.basicAuth && options.browser === 'firefox') ||
      (options.cookie && options.browser === 'firefox') ||
      (options.injectJs && options.browser === 'firefox') ||
      (options.clearCacheKeepCookies && options.browser === 'firefox')
    ) {
