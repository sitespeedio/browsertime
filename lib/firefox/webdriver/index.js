...
module.exports.configureBuilder = function(builder, baseDir, options) {
  ...
  if (!options.skipHar) {
    ...
    // We have 61 in Docker. But stable is 60 and will fail without choosing netmonitor
    // When 61 is stable we can skip this step.
    if (!options.docker) {
    ...
    profile.addExtension(
      path.resolve(
        moduleRootPath,
        'vendor',
        'harexporttriggerbrowsertime-0.0.7-fx.xpi'
      )
    );
  }
