...
class Firefox {
  ...
  async afterEachURL(runner, index, result) {
    ...
    // We have the result with alias and URLs, lets sneak
    // that into the HARs
    if (this.hars[index - 1].log.pages.length > 0) {
      // Hack to add the URL from a SPA
      if (result.alias && !this.aliasAndUrl[result.alias]) {
        this.aliasAndUrl[result.alias] = result.url;
        this.hars[index - 1].log.pages[0]._url = result.url;
      } else if (result.alias && this.aliasAndUrl[result.alias]) {
        this.hars[index - 1].log.pages[0]._url = this.aliasAndUrl[result.alias];
      } else {
        this.hars[index - 1].log.pages[0]._url = result.url;
      }
    }
