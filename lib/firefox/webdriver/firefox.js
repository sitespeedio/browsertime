...
class Firefox {
  ...
  async beforeStartIteration(runner) {
    if (this.firefoxConfig.appendToUserAgent) {
      const currentUserAgent = await runner.runScript(
        'return navigator.userAgent;',
        'GET_USER_AGENT'
      );
      let script = `Services.prefs.setStringPref('general.useragent.override', '${currentUserAgent} ${this.firefoxConfig.appendToUserAgent}');`;
