...
export class GeckoProfiler {
  ...
  async start() {
    ...
    if (
      (firefoxConfig.geckoProfiler || firefoxConfig.geckoProfilerCustom) &&
  ...
  async stop(index, url) {
    ...
    const options = this.options;
    // automatically set the index and url when using the exposed
    // geckoprofiler commands
    if (index == undefined && this.firefoxConfig.geckoProfilerCustom) {
      index = 1;
    }
    if (url == undefined && this.firefoxConfig.geckoProfilerCustom) {
      url = options.browsertime.url;
    }
