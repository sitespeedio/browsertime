...
### Added
* Added unified selector syntax: `commands.click('#btn')`, `commands.click('id:myId')`, `commands.click('text:Submit', { waitForNavigation: true })`. Supports prefixes: `id:`, `xpath:`, `text:`, `link:`, `name:`, `class:` — CSS selector is the default. The `AndWait` methods are deprecated in favour of `{ waitForNavigation: true }` [#2390](https://github.com/sitespeedio/browsertime/pull/2390).
* Added `commands.type(selector, text)` as a shorthand for `addText.bySelector(text, selector)` with a more conventional parameter order [#2387](https://github.com/sitespeedio/browsertime/pull/2387).
* Added `commands.find(selector, options)` helper that combines element lookup with optional waiting and visibility checks. Uses the configured `--timeouts.elementWait` as default timeout [#2386](https://github.com/sitespeedio/browsertime/pull/2386).
...
* Added `commands.click.byText(text)` and `byTextAndWait(text)` to click any element by visible text, not just links [#2384](https://github.com/sitespeedio/browsertime/pull/2384).
...
* Added `actions.clear()` after `perform()` in all mouse commands (singleClick, doubleClick, contextClick, clickAndHold) to prevent pointer/key state leaking between actions [#2407](https://github.com/sitespeedio/browsertime/pull/2407).
